...
class PostScheduler:
    ...
    async def auto_post_cycle(self):
        ...
        try:
            ...
            # Проверяем на критические ошибки, требующие отключения автопостинга
            critical_errors = [
            ...
            is_critical = any(keyword in error_str for keyword in critical_errors)
            
            if is_critical:
                # Критическая ошибка - отключаем автопостинг
                logging.error("[Auto-Post] 🚨 Критическая ошибка! Отключаем автопостинг для предотвращения зацикливания")
                await update_setting("auto_mode_status", "off")
                await update_setting("auto_posting_enabled", 0)
                ...
                try:
                    await self.bot.send_message(
                        ADMIN_ID,
                        f"🚨 <b>КРИТИЧЕСКАЯ ОШИБКА - АВТОПОСТИНГ ОТКЛЮЧЕН</b>\n\n"
                        f"❌ <b>Проблема:</b> {str(e)[:200]}...\n\n"
                        f"🛑 <b>Автопостинг автоматически отключен</b> для предотвращения повторения ошибки.\n\n"
                        f"💡 <b>Возможные причины:</b>\n"
                        f"• Географические ограничения OpenAI\n"
                        f"• Проблемы с API ключом\n" 
                        f"• Превышена квота или недостаточно средств\n\n"
                        f"🔧 <b>Что делать:</b>\n"
                        f"1. Проверьте настройки OpenRouter API\n"
                        f"2. Смените модель на доступную в вашем регионе\n"
                        f"3. Включите автопостинг заново через меню бота\n\n"
                        f"💡 <i>Рекомендуется использовать модель deepseek/deepseek-r1:free</i>",
                        parse_mode='HTML'
                    )
                except Exception as notify_error:
                    logging.error(f"[Auto-Post] Не удалось отправить уведомление о критической ошибке: {notify_error}")
                
                return
            
            elif any(keyword in error_str for keyword in ['openrouter', 'connection', 'timeout', 'api']):
                # Обычная ошибка API - продолжаем попытки
                try:
                    await self.bot.send_message(
                        ADMIN_ID,
                        f"⚠️ <b>Временная ошибка автопостинга</b>\n\n"
                        f"🔄 Не удалось подключиться к OpenRouter.ai после нескольких попыток.\n\n"
                        f"📝 <b>Тема:</b> {topic.theme}\n"
                        f"🏷️ <b>Категория:</b> {topic.category}\n"
                        f"❌ <b>Ошибка:</b> {str(e)[:200]}...\n\n"
                        f"💡 <i>Автопостинг продолжит работу при следующем цикле.</i>",
                        parse_mode='HTML'
                    )
                except Exception as notify_error:
                    logging.error(f"[Auto-Post] Не удалось отправить уведомление администратору: {notify_error}")
