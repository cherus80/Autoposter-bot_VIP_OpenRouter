...
## [2024-03-27] - Исправление проблем с OpenRouter API

### Исправлено
- Исправлена проблема с авторизацией OpenRouter API
- Обновлены настройки моделей OpenRouter:
  - POST_MODEL = 'deepseek/deepseek-r1:free'
  - IMAGE_PROMPT_MODEL = 'deepseek/deepseek-r1:free'
- Увеличено количество попыток повторного подключения до 5

### Изменено
- Убраны переменные окружения по умолчанию из docker-compose.yml
- Жестко заданы модели OpenRouter в docker-compose.yml для предотвращения конфликтов
- Обновлена конфигурация retry механизма в config.py
