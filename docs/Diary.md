...
## [2024-03-27] - Исправление проблем с импортом модулей и путями

### Добавлено
- Создан файл `__init__.py` в директории database для корректной работы импортов
- Добавлен том `bot_logs` для хранения логов

### Изменено
- Обновлены пути к базам данных в `bot.py`:
  - FSM хранилище перемещено в `/app/database/fsm_storage.db`
- Оптимизирована конфигурация томов в `docker-compose.yml`:
  - Заменены локальные пути на именованные тома
  - Добавлен том для логов
  - Удален неиспользуемый том `bot_data`
- Добавлена переменная `OPENROUTER_IMAGE_PROMPT_MODEL` в конфигурацию

### Исправлено
- Исправлена проблема с импортом модуля `database.database`
- Унифицированы пути к базам данных и хранилищам
